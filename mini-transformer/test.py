...
def test_encoder():
    ...
    enc = Encoder(num_layers=3, d_model=d_model, n_heads=4, dropout=0.1, d_ff=64)
    ...
    torch.manual_seed(0)
    enc0 = Encoder(num_layers=3, d_model=d_model, n_heads=4, dropout=0.0, d_ff=64)
...
def test_encoder_layer_norm_is_last():
    enc = Encoder(num_layers=1, d_model=16, n_heads=2, dropout=0.0, d_ff=32)
