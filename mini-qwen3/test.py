...
def _mask(seq_len: int, total: int | None = None):
...
def test_tf_block_kv_cache_passthrough():
    ...
    out_full, _ = block(x, _mask(5), sin, cos)

    cache = None
    for t in range(5):
        x_t = x[:, t : t + 1]
        out_t, cache = block(x_t, _mask(1, t + 1), sin, cos, kv_cache=cache)
...
if __name__ == "__main__":
    for fn in (
        ...
        test_gqa_gradient,
        test_tf_block_pre_norm_residual,
        test_tf_block_kv_cache_passthrough,
        test_qwen3_prefill_and_generate,
        test_qwen3_kv_cache_consistency,
        test_qwen3_structure_matches_config,
        test_qwen3_gradient,
    ):
        fn()
